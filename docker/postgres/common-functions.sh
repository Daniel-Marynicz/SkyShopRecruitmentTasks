...
app_test_user="${APP_TEST_USER:-$(test -f "${APP_TEST_USER_FILE}" && cat ${APP_TEST_USER_FILE} || echo "")}"
app_test_password="${APP_TEST_USER_PASSWORD:-$(test -f "${APP_TEST_USER_PASSWORD_FILE}" && cat ${APP_TEST_USER_PASSWORD_FILE})}"
app_test_db="${APP_TEST_DB:-$(test -f "${APP_TEST_DB_FILE}" && cat ${APP_TEST_DB_FILE} || echo "")}"
